...
/// Returns (jobs, preamble_lines). Preamble lines are non-job lines (env vars,
/// blank lines, plain comments) that get preserved when writing back.
/// Lines like `# 0 7 * * * /path/to/cmd` are parsed as disabled jobs.
    ...
    if let Ok(out) = output {
        ...
        for line in content.lines() {
            ...
            if trimmed.starts_with('#') {
                // Could be a disabled job: `# <schedule> <command>`
                let rest = trimmed[1..].trim();
                if let Some(job) = try_parse_job(id, rest, false) {
                    jobs.push(job);
                    id += 1;
                    continue;
                }
            ...
            if let Some(job) = try_parse_job(id, trimmed, true) {
                jobs.push(job);
                id += 1;
            } else {
                preamble.push(line.to_string());
...
fn try_parse_job(id: usize, line: &str, enabled: bool) -> Option<CronJob> {
...
struct CronManagerApp {
    jobs: Vec<CronJob>,
    /// Non-job lines from the crontab (env vars, comments, blanks) preserved on write-back.
    crontab_preamble: Vec<String>,
    /// When true, never write back to crontab.
    use_mock: bool,
...
impl CronManagerApp {
    ...
    fn save_crontab(&mut self) {
        ...
        if let Err(e) = write_crontab(&self.crontab_preamble, &self.jobs) {
            self.status_msg = Some(format!("Error: {e}"));
        } else {
            self.status_msg = Some("Crontab saved.".into());
        }
    }
}

impl eframe::App for CronManagerApp {
            ...
            .show(ctx, |ui| {
                ...
                if let Some(msg) = &self.status_msg {
                    ...
                    let footer = if self.use_mock {
                        "Mock mode · changes not saved to crontab"
                    } else {
                        "Powered by crontab · Click a job to expand"
                    };
                    ui.vertical_centered(|ui| {
...
impl CronManagerApp {
    fn render_header(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            ui.vertical(|ui| {
                let title = if self.use_mock { "Cron Manager (mock)" } else { "Cron Manager" };
                ui.label(egui::RichText::new(title)
                    .size(20.0).strong().color(egui::Color32::from_rgb(241, 245, 249)));
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                if ui.add(egui::Button::new(
                    egui::RichText::new("+ New Job").size(12.0).color(egui::Color32::WHITE))
                    .fill(Theme::ACCENT)
                    .corner_radius(8.0)
                ...
                if ui.add(egui::Button::new(
                    egui::RichText::new("↻ Refresh").size(12.0).color(Theme::TEXT_MUTED))
                    .fill(Theme::PANEL)
                    ...
                    if self.use_mock {
                        ...
                        let (jobs, preamble) = parse_crontab();
                    ...
                    .show(ui, |ui| {
                        ui.horizontal(|ui| {
                            ...
                            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                                ...
                                let toggle_text = if job.enabled { "◉" } else { "○" };
                                let toggle_color = if job.enabled { Theme::SUCCESS } else { Theme::TEXT_DIM };
                                if ui.add(egui::Button::new(
                                    egui::RichText::new(toggle_text).size(14.0).color(toggle_color))
                                    .fill(egui::Color32::TRANSPARENT)
                                    .stroke(egui::Stroke::new(1.0, Theme::BORDER))
                                    .corner_radius(6.0)
                                    .min_size(egui::vec2(28.0, 28.0))
                            ...
                            egui::Frame::new()
                                ...
                                .show(ui, |ui| {
                                    ui.horizontal_wrapped(|ui| {
                                        ...
                                        ui.label(egui::RichText::new(&job.command)
                                            ...
                                            ui.vertical(|ui| {
                                                ui.horizontal(|ui| {
                                                    ...
                                                    ui.label(egui::RichText::new(ts)
                                                        .size(11.0).color(Theme::TEXT_DIM));
                                                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                                                        ui.label(egui::RichText::new(&log.duration)
                                                            .size(11.0).color(Theme::TEXT_DIM));
                                                    });
                                                });
                                                ...
                                                ui.label(egui::RichText::new(&log.output)
                                                    .size(12.0).color(out_color));
                                            });
                                        });
                                    });
                            }
        ...
        egui::Window::new("New Cron Job")
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui.add(egui::Button::new(
                        egui::RichText::new("Add Job").color(egui::Color32::WHITE))
                        .fill(Theme::ACCENT)
                        .min_size(egui::vec2(80.0, 30.0))
                    ).clicked() {
                        if !self.new_name.is_empty() && !self.new_schedule.is_empty() && !self.new_command.is_empty() {
...
fn main() -> eframe::Result<()> {
    ...
    eframe::run_native(
        ...
        Box::new(move |_cc| {
            let app = if use_mock {
                CronManagerApp::new(sample_jobs(), Vec::new(), true)
            } else {
                let (jobs, preamble) = parse_crontab();
                let jobs = if jobs.is_empty() { sample_jobs() } else { jobs };
                CronManagerApp::new(jobs, preamble, false)
            };
            Ok(Box::new(app))
        }),
    )
